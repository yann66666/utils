...
//return :      事件触发器
//***************************************************
func (trigger *Trigger) AddListener(event interface{}, args ...interface{}) *Trigger {
	// 加锁
	trigger.lock.Lock()
	defer trigger.lock.Unlock()

	for i := 0; i < len(args); i++ {
		// 反射回调函数
		fn := reflect.ValueOf(args[i])
		...
		if reflect.Func != fn.Kind() {
			...
			if nil == trigger.recoverer {
				...
				trigger.recoverer(event, args[i], ErrNotFunction)
			}
		}
		...
		if trigger.maxListeners != -1 && trigger.maxListeners < len(trigger.events[event])+1 {
			if nil == trigger.recoverer {
				...
				trigger.recoverer(event, args[i], ErrExceedMaxListeners)
			}
		}
...
//return :      事件触发器
//***************************************************
func (trigger *Trigger) On(event interface{}, listener ...interface{}) *Trigger {
